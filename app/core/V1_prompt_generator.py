# app\core\V1_prompt_generator.py
import json
import math
import os
import re
...
# التعديل الأول: وصف الخلفية والأرضية (Beige Aged Paper)
REQUIRED_OPENING = (
    "Editorial collage illustration in watercolor style. "
    "A clean, singular focal point on a full-frame background of heavy, aged beige parchment paper. "
    "The entire background is a solid, textured antique paper, sand-colored with amber stains."
)

# التعديل الثاني: منع الأقواس وضمان الألوان
REQUIRED_STYLE_TAGS = (
    " Large bold text without brackets in a heavy vintage serif font, off-white with dark shadow. "
    "A thin, vibrant red hand-drawn ink line. Sharp focus, minimalist composition. "
    "NO WHITE BACKGROUND, NO WHITE MARGINS, 100% COVERED IN BEIGE PAPER TEXTURE."
)

def _fix_scene(scene: dict) -> dict:
    prompt = scene.get("main_prompt", "")
    label = scene.get("label_text", "").replace("[", "").replace("]", "") # حذف الأقواس لو وجدت
    
    # التأكد من البداية
    if not prompt.startswith("Editorial collage illustration"):
        prompt = REQUIRED_OPENING + " " + prompt

    # استبدال أي [LABEL] أو أقواس داخل البرومبت نفسه
    prompt = prompt.replace("[", "").replace("]", "")

    # التأكد من الـ Style Tags
    if "vibrant red" not in prompt.lower():
        prompt = prompt.rstrip(".") + "." + REQUIRED_STYLE_TAGS

    # الـ Negative Prompt (تشديد على منع الأبيض)
    scene["negative_prompt"] = (
        "white background, plain white floor, white margins, white corners, white frame, "
        "empty white space, photographic realism, 3D render, modern digital art, "
        "crowded composition, messy layout, anime, neon"
    )

    scene["main_prompt"] = prompt
    scene["label_text"] = label # تحديث الليبل النظيف
    return scene
...
def generate_prompts(
    ...
    # ✅ هنا بعد ما all_scenes و resume_from اتعرفوا
    log(f"DEBUG: resume_from={resume_from} | total_images={total_images} | all_scenes={len(all_scenes)}")
    log(f"DEBUG: script words={len(script.split())} | scenes_per_batch={scenes_per_batch}")
    ...
    for i in range(total_batches):
        ...
        for s in batch:
            ...
            s = _fix_scene(s)  # ← auto-fix: يضمن الـ opening وإزالة الخلفية البيضاء
            all_scenes.append(s)
            scene_counter += 1
    ...
    for i, s in enumerate(all_scenes, start=1):
        txt_lines.append(s.get("main_prompt", "").strip())

        if i % 10 == 0 and i != len(all_scenes):
            txt_lines.append("")
            txt_lines.append("-" * 80)
            txt_lines.append("")
